...
func (k Keeper) GetValidatorWhitelist(ctx sdk.Context) (whitelist []sdk.ValAddress) {
	store := ctx.KVStore(k.storeKey)
	b := store.Get(ValidatorsWhitelistKey)
	if b == nil {
		return nil
	}
	k.cdc.MustUnmarshalBinaryLengthPrefixed(b, &whitelist)
	return
}

func (k Keeper) SetValidatorWhitelist(ctx sdk.Context, whitelist []sdk.ValAddress) {
	store := ctx.KVStore(k.storeKey)
	b := k.cdc.MustMarshalBinaryLengthPrefixed(whitelist)
	store.Set(ValidatorsWhitelistKey, b)
}
