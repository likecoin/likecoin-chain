...
import (
	...
	keep "github.com/likecoin/likechain/x/staking/keeper"
...
func NewQuerier(k keep.Keeper, cdc *codec.Codec) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err sdk.Error) {
		...
		case QueryWhitelist:
			return queryWhitelist(ctx, cdc, k)
		default:
...
func queryWhitelist(ctx sdk.Context, cdc *codec.Codec, k keep.Keeper) (res []byte, err sdk.Error) {
	whitelist := k.GetValidatorWhitelist(ctx)

	res, errRes := codec.MarshalJSONIndent(cdc, whitelist)
	if errRes != nil {
		return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", errRes.Error()))
	}
	return res, nil
