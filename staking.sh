...
ADDRESS=$(docker exec likechain_gaiad gaiacli keys show validator -a)
echo "Address: $ADDRESS"

VAL_PUBKEY=$(docker exec likechain_gaiad gaiad tendermint show-validator)
...
BALANCE=$(docker exec likechain_gaiad gaiacli query account "$ADDRESS" --chain-id $CHAIN_ID | grep "^\s*Coins" | sed 's/ *Coins: *\([0-9][^ ]*\)/\1/g')
...
docker exec -it likechain_gaiad \
